...
| `approval_not_stale` | no file was edited in a commit later than the one that set its marker | Markers carry no dates — git already keeps them, so nobody types one and nobody bumps one. **Commit granularity is the whole check, not a refinement:** the date-based version reported clean on a day when the entire corpus was written, approved and committed, while `names.md`, `safety-suits.md` and `humour-plan.md` had each been edited in a later commit than the one that approved them. Uncommitted edits are invisible here; every other check reads the working tree. |
