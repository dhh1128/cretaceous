...
| `readme_matches_frontmatter` | the README approval table agrees with the files' own headers | They disagree for five files, so "which files are approved" has no single answer. **The right fix is to generate the table and retire this check.** |
| `us_english` | no British spellings | `AGENTS.md` §5. A literary register pulls toward them and it keeps coming back. |
| `lingo_closed_list` | the closed vocabulary holds the number of terms the corpus says it holds | It holds sixteen and four files say fifteen, including the file itself. It has never held fifteen — `git show` returns sixteen at every commit that ever touched it. |
