...
`questions.py` answers *what is still open, and what did he already answer that nobody propagated.* The second half is why it exists. `Q-8HNV` sat in `scene-build-runbook.md` marked **unanswered** for five days after Daniel had answered it — *"what was written about it in the superseded 1.x scenes is what I intended to be true"* — and a later session re-derived a different answer from scratch. **An unanswered question costs a round trip; an answered one nobody can find costs the same round trip and his patience with it.**

**It is the first instrument here that reads `.ignored/`.** Every other one reads `git ls-files`, so the ruling records have been structurally invisible to the suite since they were written. For each section that records a decision — a decision-verb heading, or a body that quotes him making one — it asks whether the section's own emphasised phrases are findable in the tracked corpus. **A lead and not a verdict**: canon rephrases, so an absent phrase means go and look.

**Two rounds of tuning are recorded in its docstring**, because both are the same mistake this directory keeps making. Reading every `##` section reported 148 of them and drowned the signal — `allocation_off_day`'s failure, repeating. And the open half originally *ranked* items by how many files mention what they name, which turned out to measure how common the words were: three notes about cover art outscored the scene-ledger items blocking work. **The ranking is gone rather than improved.** It groups by whose ground an item sits on, which is computable, and says outright that dependency order is a reader's job.
...
| `no_legacy_addresses` | no file cites a scene by its retired outline-unit address | **The renumber's permanent verifier.** 261 references moved from `N.N` to `D<day>.<n>`, and four of the fourteen Act 1 mappings kept their digits — `2.3` became `D2.3` — so a missed reference looks almost right rather than wrong. Green, and it stays, because the failure it guards is somebody writing `2.3` from memory next month. |
...
| `scene_entry_complete` | every scene entry carries a size band, a day, a POV, a place, a `**Ladders:**` line and an `**Ends on:**` line | A field an entry does not carry is a field no other check can read, and the failure is silent — `scenes_per_day` read the day-keyed entries as absent for a whole rescene because nothing had widened the pattern that finds them. **Catches 24 today, in two distinct groups:** twelve Act 2 entries still in the older single-line form, which carry no size band, no ladders and no ending and are being converted now; and **twelve Act 1 entries — D1.1, D2.1, D2.2, D2.4, D2.5, D2.6 through D2.9, D2.10 through D3.1 — that carry everything else and no `**Ends on:**`**, which is not the conversion and is nobody's known work item. `**Hazard:**` is deliberately not required: it arrived after the format did, and the 26 entries that predate it are not defective for lacking it. |
...
| `payment_order` | no ledger row pays before the scene that plants it | A plant only works forward, and the ledger is what a drafting pass reads to decide what has to exist first, so a reversed row sends the writing at it backwards. **Caught one and it is fixed:** row 21 planted the river gratings at the yazhi-pool drill and paid them at the family nest that morning — and two other files already knew the manner of Yara's death belongs at the Repository that night. Green now, and a regression guard. **Ordering is by position in the scene list, not by parsing an id** — see the docstring, which records why. |
| `ending_distribution` | scene endings match the measured shape for the job each scene is doing | `pacing-and-stakes.md` §8. **The check the hooks layer exists to make possible**, and the one that retires §4's flat *every scene ends on a hook* — which would write the thirteen BOND scenes wrong in a predictable direction, since a relationship chapter in Daniel's own novels settles 57% of the time. Green today at 64.3% against a measured 62.2%, and the probe that matters is that flipping every ending to a hook fires all five policed classes and the aggregate. See "What it does not police" below |
...
### What `ending_distribution` does not police, and the one hole left in it

**Eight of the seventy scenes are tagged and unchecked**, for two different reasons that are worth keeping apart. DECIDE has three scenes, below the floor at which any assignment would satisfy any band. CONSOLIDATE, OPEN and CLOSE have six, four and two *chapters* behind them in the measurement, and its own source says those rows carry no information — so there is no rate to check them against, and inventing one here would be the suite enforcing a decision nobody made. Both floors are declared in `pacing-and-stakes.md` §8d and read at runtime.

**The hole: nothing notices a class sliding under a floor.** If purposes are reassigned during drafting and CLASH drops from eight scenes to four, the check stops policing it and stays green, which is the shape of failure this directory has already been bitten by — `no_legacy_addresses` read its own subject from the notes it was about to lose. The guard that would close it is the `act_composition` pattern: §8f states the composition, and a check compares it. **That is not written**, because §8f would then have to be edited on every reassignment, and the composition is going to move while the scene maps are built. Revisit when the maps are done and the purposes settle.

**And the measurement counts chapters while this counts scenes.** §8e has the argument for why the conditional form transports anyway and the flat book-level rate does not. When chapters are drawn, the check wants a second run over chapter-final scenes only, which is the population that matches what was measured.
...
- **The scene list today** runs 70 scenes in 50 runs — 38 of one scene, nine of two, one of three, **one of five** (KEO, D2.8 through D3.1, which is the theft) and **one of six** (BENAL, D13.1 through D15.1, which is the drag-frame, the coast, the raft and the coral bank).
