...
- **2026-09-14** — **The first two forward maps of a scene that has no prose: D12.1 and D12.2.** C11 added and run once. Nineteen ledger items put to Daniel against the D2.3 baseline of zero. **The three flywheel numbers, and the second and third are not measurable yet because he has not read the maps** — what is measurable is the other direction: an eighty-word message from him resolved three items, including the one that had looked like a choice between two contradictory latency figures and turned out to be a dose distinction that neither figure was wrong about. That ratio is the whole argument for the ledger and it is the first real data point for C10.
  ...
  **Three process findings.** The mechanical gate belongs at phase 3 and not only at phase 7: run against the maps, it caught a citation to a section `voice-sheets.md` does not have and an instance of the eliminated unit word, three phases before the runbook puts the check. **An unresolved `requires` really does function as a work queue** — `foreshadow-and-motif.md` **R18** plants a stone-cycad correction in "Act 1" and names no scene, nothing in the scene list carries it, and the map could not be written without noticing. And **walking the invariants caught two `[requires]` payloads that address resolution would have passed**: a pointer at Noli's death does not carry *she was the protein*, and the projected register is established at D10.3 rather than D2.1. That is §5.4 doing exactly what it was added for, on its first real use.
