...
**C11. An adversarial outside read of a forward map finds a class of defect the four invariants structurally cannot. — Provisional.**
Run once, on D12.1 and D12.2, against a self-contained packet: the framing, the canon the maps rest on, and the maps. Four models from four lineages, each blind to the others.

**The two findings that reshaped the scenes were both absences, and neither is reachable from §5.** D12.2 was a night camp in three days of rain with nothing dry to burn and an unsuited POV, and four of its moves required him to see something. There was no light in it at all. Three of the four seats found that independently in one pass. And D12.2's turn rested on Keo's proximity sense failing to warn him about a hole in the ground, when the sense reads *presence and kind* of living things and has never been able to read terrain.

**Why the invariants cannot get there.** All four of them check the map against something the map already points at — the neighboring scenes, the knowledge ledger, its own `[requires]` addresses. **Nothing was checking the map against the physical world**, and the artifact that is supposed to is the invention ledger, which by construction contains only what its author noticed. An author cannot list what they failed to see, because knowing what they meant is exactly what conceals the gap. A reader holding the same canon and none of the intention is a different instrument, and it is cheap.

**Falsifier:** run it on the next two maps. If the outside read returns only items already on the ledger, or only noise, the stage does not earn its cost.
...
- **2026-09-14** — **The first two forward maps of a scene that has no prose: D12.1 and D12.2.** C11 added and run once. Nineteen ledger items put to Daniel against the D2.3 baseline of zero. **The three flywheel numbers, and the second and third are not measurable yet because he has not read the maps** — what is measurable is the other direction: an eighty-word message from him resolved three items, including the one that had looked like a choice between two contradictory latency figures and turned out to be a dose distinction that neither figure was wrong about. That ratio is the whole argument for the ledger and it is the first real data point for C10.

  **What the outside read actually moved, since the maps no longer carry it** (and see the next paragraph for why they no longer carry it). D12.2 had no light in it at all — night camp, three days of rain, nothing dry to burn, an unsuited POV, and four moves requiring him to see. Three of four seats found it independently. D12.2's turn rested on the proximity sense warning Keo about a hole in the ground, which it has never been able to do; it now fails on the animal, which is what the sense is for. The leach was three hours rather than four, because the prep stood between the opening and the bag entering the water. Teva's decision was a gamble on the leach having finished and is now a trade against a known partial dose. Her venom resistance was claimed as the best of the three, which canon does not support. Benal's objection had no content, so the hedge had nothing to weaken. The four-hour lay-up contained none of the three tasks `body-and-resources.md` §2b assigns to a lay-up. And Teva was furniture in D12.2. **One seat's headline was confidently wrong** — that a sulfidic spring in limestone runs pH 2–3 — and it led that seat's verdict.

  **Then two corrections from the author that the panel did not reach, and the second dissolved findings rather than fixing them.** The four-hour window had no justification and the obvious move was to camp at the spring: the answers are that a leach needs flow rather than immersion, so nobody leaches while walking, and that a warm spring is the best water for kilometers and therefore the worst ground to sleep on. And **the remainder goes into the runoff overnight** — Keo's own method, adopted a few hours after he was overruled, for everybody except the person who overruled him. That is the differential the whole of Day 13 rests on, and it is cleaner than the dose count that preceded it: she is not sicker because she ate more, she is sicker because she ate the batch that had not finished, and she ate it because she would not let anyone else go first. It also **dissolved** two of the panel's findings without a fix — the *Didelphodon*'s motive needs no invention once the mash is in a channel at night, and "why do three starving people sit beside food they do not eat" answers itself.

  **`dated_provenance` caught the maps carrying their own history, and it was right to.** Scene maps live under `plan/`, so `AGENTS.md` §2 applies: knowledge files carry what is true, and git carries how it got there. Revision banners, "what the red team moved" sections and per-move notes of the form *revision 1 had X* are all provenance, and the check fired on the most explicit of them. **The rule that came out of it: a map's rationale must be stated as a standing constraint rather than as a correction** — not *revision 1 had the proximity sense reading terrain*, but *the sense has never read terrain*. The first is history and drifts; the second is a prohibition and holds. All of it moved here, which is the one file where history is the subject.

  **Three process findings.** The mechanical gate belongs at phase 3 and not only at phase 7: run against the maps, it caught a citation to a section `voice-sheets.md` does not have and an instance of the eliminated unit word, three phases before the runbook puts the check. **An unresolved `requires` really does function as a work queue** — `foreshadow-and-motif.md` row 18 plants a stone-cycad correction in "Act 1" and names no scene, nothing in the scene list carries it, and the map could not be written without noticing. And **walking the invariants caught two `[requires]` payloads that address resolution would have passed**: a pointer at Noli's death does not carry *she was the protein*, and the projected register is established at D10.3 rather than D2.1. That is §5.4 doing exactly what it was added for, on its first real use.
